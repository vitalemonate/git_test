...
`git config –global user.email "952070377@qq.com"`
...
`git remote add origin git@github.com:vitalemonate/learngit.git`
...
# 使用SSH方式推送文件至github仓库的方法

生成SSH密钥对

`ssh-keygen -t rsa -C "952070377@qq.com"`

执行以上指令后会在用户的home目录下生成两个文件

* 私钥文件：`/wanghao/.ssh/id_rsa`

* 公钥文件： `/wanghao/.ssh/id_rsa.pub`
...
![avatar](https://github.com/vitalemonate/learngit/blob/main/pics/5.png)
...
## 删除文件
删除工作区文件，并且将这次删除放入暂存区, 注意要删除的文件是`没有修改过的`，就是说和当前版本库文件的内容相同
...
### 恢复现场
恢复，但是恢复后，stash内容并不删除，需要用`git stash drop`来删除
...
如果`git pull`提示`no tracking information`，则说明本地分支和远程分支的链接关系没有创建，用命令`git branch --set-upstream-to branch-name origin/branch-name`创建本地分支和远程分支的链接关系
***
# 标签管理
